package eapli.base.twinServerAPI;
...
public class TwinServerProxy {
    private static final Logger LOGGER = LogManager.getLogger(TwinServerProxy.class);
    ...
    public boolean connectDigitalTwinServer(){
    ...
    public boolean connectDigitalTwinServer(Integer port){
        socket = new ClientSocket();
        try {
            socket.connect(getAddress(), port);
            return true;
        } catch (IOException e) {
            return false;
        }
    ...
    public boolean disconnectDigitalTwinServer(){
        try {
            socket.stop();
            return true;
        } catch (IOException e) {
    ...
    public String currentLocation() throws IOException, FailedRequestException {

        //Current location code is 3 and it sends no data
        byte[] requestBytes = buildRequest(3, "");
        System.out.println("\nSending Code 3: Current Location Request\n");
    ...
    public String currentStatus() throws IOException, FailedRequestException {

        //Current status code is 4 and it sends no data
        byte[] requestBytes = buildRequest(4, "");
        System.out.println("\nSending Code 4: Current Status Request\n");
